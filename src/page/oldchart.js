...
function trianglePoints(x, y, size, direction) {
  ...
  let points;
  if (direction === "up") {
    points = `${x},${y - halfSize} ${x + halfSize},${y + halfSize} ${x - halfSize
      },${y + halfSize}`;
...
function findMinMaxAndIntermediate(yScaleKeys) {
  ...
  const intermediatePoints = allValues.filter(
...
function updateDataDomain1(yscaletag, newDomain) {
  ...
  const scaleConfigToUpdate = this.yScaleConfig.find(
    (config) => config.yscaletag === yscaletag
  );
...
function arrangeData(stockData, funda = 0) {
  ...
  const shareholdingData = Object.keys(fundaDataobject).includes(
    "shareholdingData"
  )
  ...
  const Operating_Profit1 = Object.keys(profitLossData).includes(
    "Operating Profit"
  )
  ...
  return {
    xdata: xdata, // Array of timestamps for xdata
    xdata1: [0, 15, 35, 45],
    //xdata2:[0,timestamp.length],
    xdata2: [timestamp[0], timestamp[timestamp.length - 1]],
    timestamp: timestamp,
    line1: [30, 60, 50, 20],
    open: open, // Array of open prices
    high: high, // Array of high prices
    low: low, // Array of low prices
    close: close, // Array of close prices
    ema: TechnicalIndicators.EMA, // Placeholder for EMA calculation function name
    sma: TechnicalIndicators.SMA, // Placeholder for SMA calculation function name
    rsi: TechnicalIndicators.RSI,
    atr: TechnicalIndicators.ATR,
    adx: TechnicalIndicators.ADX,
    ohlckeyvlaue: TechnicalIndicators.OHLCKEYVALUE,
    volume: volume, // Array of volume values
    ...
    breakoutlines: () =>
      Object.keys(zigzagdata).includes("brlist")
...
function getYAxisTag(yScaleConfig, yscaletag) {
  const yAxisConfig = yScaleConfig.find(
    (config) => config.yscaletag === yscaletag
  );
...
function groupButtonsByPanel(buttonObjects) {
  ...
  const groupedButtonObjects = filteredButtonObjects.reduce(
    (grouped, button) => {
      ...
      // Check if it's an array of objects with properties x, y, and z
        ...
        data.forEach((item) => {
          Object.keys(item).forEach((key) => {
            ...
            if (!convertedData[mappedKey]) convertedData[mappedKey] = [];
...
class CandlestickChart {
  constructor(stockData, svgWidth, svgHeight, chartOptions) {
   ...
   // console.log(chartOptions.chartsettings);
    ...
    this.mdbutton = chartOptions.chartsettings?.mdbutton?.status ?? false;
    this.fiibutton = chartOptions.chartsettings?.fiibutton?.status ?? false;;
    this.opbutton = chartOptions.chartsettings?.opbutton?.status ?? false;;
    this.epsbutton = chartOptions.chartsettings?.epsbutton?.status ?? false;;
    this.brlinebutton = chartOptions.chartsettings?.brlinebutton?.status ?? false;;
    this.crsibutton = chartOptions.chartsettings?.crsibutton?.status ?? false;;
    this.adxbutton = chartOptions.chartsettings?.adxbutton?.status ?? false;;
    this.atrbutton = chartOptions.chartsettings?.atrbutton?.status ?? false;;
    this.emabutton = chartOptions.chartsettings?.emabutton?.status ?? false;;
    this.rsibutton = chartOptions.chartsettings?.rsibutton?.status ?? false;;
    this.trendbutton = chartOptions.chartsettings?.trendbutton?.status ?? false;;
    this.zigzagbutton = chartOptions.chartsettings?.zigzagbutton?.status ?? false;;
    this.closebutton = chartOptions.chartsettings?.closebutton?.status ?? false;;
    this.cdbutton = chartOptions.chartsettings?.cdbutton?.status ?? true;;
    this.volbutton = chartOptions.chartsettings?.volbutton?.status ?? false;;
    this.sigbutton = chartOptions.chartsettings?.sigbutton?.status ?? false;;
    ...
    this.xScaleConfig = {
      ...
      bot2: {
        ...
        mappedwith: null, // just to display axis tick and reverse map if any dataplot have axis defeind in timestamp
        //scaleRole: "main",
        plotaxis: true,
        zooming: false,
      },
    ...
    this.yScaleConfig = {
      ...
      BR: {
        ...
        maxscaledata() {
          // Filter high values within the visrange
          const highWithinRange = this.scaledata().filter(
            (d, i) => i >= this.visrange()[0] && i <= this.visrange()[1]
          );
      ...
      BL: {
        ...
        maxscaledata() {
          // Filter high values within the visrange
          if (this.scaledata().length === 2) return this.scaledata()[1];
          const highWithinRange = this.scaledata().filter(
            (d, i) => i >= this.visrange()[0] && i <= this.visrange()[1]
          );
          return d3.max(highWithinRange, (d) => d + d * this.ypadding());
        ...
        minscaledata() {
          // Filter low values within the visrange
          if (this.scaledata().length === 2) return this.scaledata()[0];
          const lowWithinRange = this.scaledata().filter(
            (d, i) => i >= this.visrange()[0] && i <= this.visrange()[1]
          );
          return d3.min(lowWithinRange, (d) => d - d * this.ypadding());
      ...
      BB1: {
        ...
        transform: { k: 1 },
        scaledata: () =>
          this.dataset.fundaY1.flatMap((d) =>
      ...
      FNYEAR_Y: {
        ...
        transform: { k: 1 },
        scaledata: () =>
          this.dataset.Operating_ProfitY.flatMap((d) =>
            Object.values(d).filter((value, index) => index !== 0)
          ),
        changeRangeTag: false,
        visrange: () => [0, 50],
      ...
      HOLD_Y: {
        ...
        transform: { k: 1 },
        scaledata: () =>
          this.dataset.HoldingData_Y.flatMap((d) =>
            Object.values(d).filter((value, index) => index !== 0)
          ),
        changeRangeTag: false,
        visrange: () => [0, 50],
      ...
      BBBAR: {
        ...
        transform: { k: 1 },
        scaledata: () =>
          this.dataset.fundabarY1.flatMap((d) =>
            Object.values(d).filter((value, index) => index !== 0)
          ),
        changeRangeTag: false,
        visrange: () => [0, 50],
    ...
    this.datatoplot = {
      ...
      pricetobreak: {
        ...
        xdatamap: true, // true only if xdata domain [timestamp[0],...,timestamp[1]] is different from xscaletag axis domain [0,...,timestamp.length]
        linetype: "dashed",
        color: "green",
        yscalenumber: 2,
        fill: "none",
    ...
    this.buttonObjects = [
...

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "vol", classgroupName: "volume_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "cd", classgroupName: "ohlc_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "close", classgroupName: "cline_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
...

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "trend", classgroupName: "tr_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "rsi", classgroupName: "rsi_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "ema", classgroupName: "techline_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "atr", classgroupName: "atr_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "adx", classgroupName: "adx_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },

      {
        ...
        panelName: "utli-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "crsi", classgroupName: "crsi_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
...

      {
        ...
        panelName: "fund-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "eps", classgroupName: "eps_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "fund-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "op", classgroupName: "op_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "fund-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "fii", classgroupName: "fii_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },
      {
        ...
        panelName: "fund-button-panel",
        onClickHandler: () =>
          this.handleplotData({ classtag: "md", classgroupName: "mdline_" }),
        buttonclick: function () {
          return this.onClickHandler();
        },
      },

      {
        ...
        buttonclick: function () {
          const url = `https://gocharting.com/terminal?ticker=NSE:${this.stockid.split(":")[0]
            }&layout=1`;
...

      {
        ...
        buttonclick: () => {
          // console.log(this);
          const url = `https://www.screener.in/company/${this.stockid.split(":")[0]
            }/consolidated/`; // Replace this with the URL you want to open
          window.open(url, "_blank");
          // return this.onClickHandler();
        },
      },
  ...
  moveCrosshair(event) {
    ...
    if (!this.svg.select(".x-zoom").property("__zoom")) return null;
    ...
    const sectedscale0 = getXScaleTagsByDataTag(
      this.yScaleConfig,
      "yaxistag",
      yaxistag,
      "yscaletag"
    ...
    const sectedscale = sectedscale0.filter((element) =>
      this.uniqueYscaletags.includes(element)
    );

    if (sectedscale.length == 0) return null;
    ...
    const currentXscale = currentTransformX.rescaleX(
      this.xScaleConfig["bot"].xScale
    );
    ...
    //  console.log(y)
    const rightmostlimit =
      this.margin.left + this.margin.innerLeft + this.width;

    if (currentXscale(index) > rightmostlimit) return (index = index - 1);
    ...
    if (this.tooltipshow) {
      ...
      this.tooltip
        ...
        .text(
          `HL%: ${(
            (this.dataset.high[index] / this.dataset.low[index] - 1) *
            100
          ).toFixed(2)}`
        )
        .attr(
          "fill",
          this.dataset.close[index] < this.dataset.open[index] ? "red" : "blue"
        )
        .append("tspan")
        .attr("dx", "10")
        .text(
          `BL%: ${(
            (this.dataset.close[index] / this.dataset.open[index] - 1) *
            100
          ).toFixed(2)}`
        )
        .attr(
  ...
  setupYScales() {
    ...
    const totalRatio = this.yAxisConfig.reduce(
      (acc, axis) => acc + axis.ratio,
      0
    );
    ...
    const filteredYScaleConfig = Object.keys(this.yScaleConfig).filter((key) =>
      this.uniqueYscaletags.includes(key)
    );
  ...
  xaxisgenerator(xScale, xAxisObject = {}) {
    ...
    if (xmappedwith == "timestamp") {
      ...
      return axisGenerator
        .ticks(tickCount)
        .tickFormat((i) =>
          multiFormat(i, this.stockid, this.dataset[xscaledatatag])
        );
    }
    if (xmappedwith == null) {
      return axisGenerator.ticks(tickCount).tickFormat((i) => i);
  ...
  handleplotData(buttonoptios) {
    ...
    if (classgroupName === "tr_") {
      const matchedData = this.TrendLineData1.filter(
        (data) => data.tagclass === classgroupName
      );
  ...
  setupEventHandlers() {
    ...
    panelNames.forEach((panelName) => {
      ...
      const filteredpanelobject = this.buttonPanelParams.filter(
        (item) => item.className === panelName
      )[0];
  ...
  createButtonArray = (filteredpanelobject, buttonArray) => {
    // console.log(filteredpanelobject);
  ...
  renderChart() {
    ...
    for (const key in this.datatoplot) {
      if (Object.hasOwnProperty.call(this.datatoplot, key)) {
        ...
        if (config.plot) {
          this.drawdatatoPlot({ ...config, name: key });
        }
      }
    }
  ...
  renderYAxisLines() {
    ...
    const {
      min: startP,
      max: endP,
      intermediate: intermediatePointP,
    } = findMinMaxAndIntermediate(this.yAxisRange);
  ...
  drawdatatoPlot(config) {
    ...
    if (this.yScaleConfig[yscaletag] === undefined) return null;
  ...
  dbClicktoDelete(event) {
    ...
    const asscociateindex = getIndexForName(
      selectedLine_ID,
      this.TrendLineData1
    );
  ...
  drawLineScatterPlot(
    ...
    tempdata.forEach((data, index) => {
      ...
      const actualDistanceFromMidpoint = Math.min(
        maxDistanceFromMidpoint,
        Math.abs(yDomain[1] - yMidpoint)
      );

      // Calculate the offset amount to equally spread from the midpoint
      const offsetAmount =
        totalline === 1 ? 0 : actualDistanceFromMidpoint / (totalline - 1); // Subtract 1 because you don't want an offset for the first plot
      ...
      Object.keys(data).forEach((key, index2) => {
        ...
        if (key !== "x") {
          ...
          if (xdatamap) {
            ...
            const mappedx = getIndexesByValues(
              this.dataset[this.xScaleConfig[xscaletag].mappedwith],
              filteredData0.map((item) => item.x)
            );
            filteredData = filteredData0.map((item, index) => ({
              ...item,
              x: mappedx[index],
            }));
          } else {
          ...
          this.chartarea
            ...
            .attr(
              "class",
              `allplot scatter-dot scatter-dot-${name}${index}${key} ${tagclass}`
            ) // Assign classes for styling and identification
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (key === "y" || key === "z") {
          ...
          this.chartarea
            ...
            .attr(
              "class",
              `allplot line-plot-${name}${index}${key} ${tagclass}`
            )
            .attr("clip-path", `url(#clip1-${this.targetID})`)
            .attr("d", line)
            .attr("stroke", typeof color === "object" ? color[key] : color)
          ...
          if ("label" in data) {
            const avgX =
              data.x.reduce((sum, point) => sum + point, 0) / data.x.length;
            const avgY =
              data.y.reduce((sum, point) => sum + point, 0) / data.y.length;

            this.chartarea
              .append("text")
              .attr(
                "class",
                `allplot text-${name}${index} line-plot-${name}${index}${key} ${tagclass} ${tagclass}`
              )
              .attr("clip-path", `url(#clip1-${this.targetID})`)
        ...
        if (key !== "x") {
          ...
          if (currentXscale.bandwidth) {
            ...
            this.chartarea
              ...
              .attr(
                "class",
                `allplot bar bar-plot-${name}${index}${key} ${tagclass}${key} ${tagclass}`
              )
              .attr("clip-path", `url(#clip1-${this.targetID})`)
              .attr(
                "x",
                (d, i) =>
                  currentXscale(filteredData.x[i]) +
                  groupX +
                  (index1 - 1) * barwidthh
              )
              .attr("y", (d, i) => {
            ...
            this.chartarea
              ...
              .attr(
                "x",
                (d, i) => currentXscale(filteredData.x[i]) - tickwidth / 4
              )
  ...
  drawCDPlot(config, filteredData, currentXscale, currentYscale, currentyaxis) {
    const { name, color, fill, strokewidth, strokedasharray, tagclass } =
      config;
    // console.log(filteredData);
    ...
    candlesticks
      ...
      .attr("y", (d, i) => {
        ...
        return currentYscale(
          Math.max(filteredData.open[i], filteredData.close[i])
        );
      })
      .attr("width", tickwidth / 2)
      .attr("height", (d, i) => {
        // console.log(d,currentYscale(d),this.yAxisRange[currentyaxis][0]-currentYscale(d))
        return Math.abs(
          currentYscale(filteredData.open[i]) -
      ...
      .attr("fill", (d, i) =>
        filteredData.open[i] > filteredData.close[i] ? "red" : "green"
      )
      .attr("stroke", "black")
      .attr("stroke-width", 1);
  ...
  zoomedYold = (event) => {
    ...
    const yaxistag = findKeyByNumber(this.yAxisRange, y);
    // console.log(yaxistag);
    const sectedscale = getXScaleTagsByDataTag(
      this.yScaleConfig,
      "yaxistag",
      yaxistag,
      "yscaletag"
    ...
    const scaleConfigsToUpdate = this.yScaleConfig.filter(
      (scale) => scale.yscaletag === sectedscale[0]
    );
  ...
  zoomedX = (event) => {
    ...
    if (newVisibleRange["bot"].length == 0) return null;
  ...
  // Usage example:
  // Assuming you want to create a button at coordinates (100, 50) with a width of 100, height of 40,
  // displaying "Click Me", and triggering a function named handleplotData when clicked.
}
