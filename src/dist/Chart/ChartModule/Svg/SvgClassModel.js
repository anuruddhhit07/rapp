...
import { createGroupAdv, createMultipleSqure, createSVGDefs3, enhanceGroup } from "./SVGUtility";
import { Shared_ChartBaseData, Shared_ChartDimension, Shared_PlotInfo, Shared_yaxisProp, } from "../BaseSetup/SharedDataUtility";
...
class SVGClass {
    constructor() {
        ...
        this.setupSVG();
        this.BackGroup = createGroupAdv(this.svg, "back-area").drawBorder(0, 0, svgWidth, svgHeight, "red", 2, "blue", 0);
        this.BackChartGroup = createGroupAdv(this.svg, "backchart-border");
        this.FrontGroup = createGroupAdv(this.svg, "main-border").drawBorder(margin.left + margin.innerLeft, margin.top + margin.innerTop, width + margin.innerRight, height, "green", 3, "yellow", 0);
        this.ResetButton = createGroupAdv(this.svg, "reset-area")
            .translate(svgWidth - margin.right, svgHeight - margin.bottom)
            .drawBorder(0, 0, margin.right, margin.bottom, "red", 0, "white", 0.1, true);
    ...
    createYaxiseventArea(callback) {
        ...
        uniqueyaxisTag.forEach((yaxistag, index) => {
            ...
            createGroupAdv(this.svg, `yzoom yzoom-${yaxistag}`)
                ...
                .drawBorder(svgWidth - margin.right, yrange[1], margin.right, yrange[0] - yrange[1], "red", 5, "green", 0 / (index + 1))
                .call(callback);
        });
        //
    ...
    createbuttonpanel(callback, numberofbutton, buttonProp) {
        const groupedData = {};
        ...
        const initialPlotStatusArray = new Array(arrayLength).fill(false);
        const buttonsvgarray = new Array(arrayLength).fill('defaultsvg');
        const buttonidarray = [];
        ...
        this.Buttonpanel = createMultipleSqure(this.svg, "top-button-panel")
            .translate(Shared_ChartDimension.margin.left +
            Shared_ChartDimension.margin.innerLeft, 0)
            // .drawBorder(0,0,100,20,"green",3,"yellow",1)
            .createSquaresHorizontally(numberofbutton, Shared_ChartDimension.margin.top * 1.5, 2, initialPlotStatusArray, updatedIdArray, buttonsvgarray
            ...
            plotnameArray.forEach((pltname) => {
                if (Shared_PlotInfo[pltname].tooltip && Shared_PlotInfo[pltname].getTooltipHTML) {
                    ...
                    enhanceGroup(this.BackChartGroup, `tooltip tooltip-${yaxistag}-${pltname}`)
                        .translate(Shared_ChartDimension.margin.innerLeft +
                        Shared_ChartDimension.margin.left, (Shared_yaxisProp[yaxistag].range[1] + Shared_ChartDimension.margin.innerTop * index))
                        .drawBorder(0, 0, Shared_ChartDimension.svgWidth, Shared_ChartDimension.margin.innerTop, "red", 2, "blue", 0);
                }
                index++;
            });
        });
    ...
    setupSVG() {
        const { targetID, svgWidth, svgHeight, margin, width, height } = Shared_ChartDimension;
        const svgElementExists = d3.select(`#svg-${targetID}`).empty();
