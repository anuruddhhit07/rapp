...
import { Shared_ChartBaseData, Shared_ChartDimension, Shared_ChartPlotData, Shared_XScaleConfig, Shared_XYrelation, Shared_YScaleConfig, updateShared_PlotInfo, updateShared_XScaleConfig, updateShared_YScaleConfig, } from "../BaseSetup/SharedDataUtility";
import { formatVolume, multiFormat } from "./dateFormat";
import { getTooltipHTMLOHLC, getTooltipHTMLVolume } from "../Svg/ToolTipUtility";
function custumticformat(i, stockid, datatotag) {
    return multiFormat(i, "temp:1D", Shared_ChartPlotData[datatotag]);
}
function customvolumformat(volume) {
    return formatVolume(volume);
}
function xaxisgenerator(xScale, xAxisObject = { scaleSide: "Bottom", ticlavelmappedwith: "xindex" }) {
    ...
    return axisGenerator.ticks(tickCount).tickFormat((i, index) => {
...
function yaxisgenerator(yScale, yAxisObject = { scaleSide: "Left", yscaleTag: "MAIN" }) {
...
function XAxisOnSVG(scaleconfig, currentTransformXb, axisAreaonSVG) {
    if (scaleconfig.xscale != null) {
        const XSL = scaleconfig.xscale().XSCALE;
...
function YAxisOnSVG(scaleconfig, axisAreaonSVG) {
    if (scaleconfig.yscale != null) {
        ...
        axisAreaonSVG
            ...
            .call(yaxisgenerator(TransYSL, {
            scaleSide: scaleconfig.scaleSide,
            yscaleTag: scaleconfig.yscaleTag,
        }));
    }
}
export function intialRendorAxis(axisAreaonSVG, xzoomeventsvg) {
...
export function drawXaxis(axisAreaonSVG, xzoomeventsvg) {
...
export function drawYaxis(axisAreaonSVG, svg, ymousepoint) {
    const yscaleTagSet = Array.from(Shared_ChartBaseData.yscaleTag);
