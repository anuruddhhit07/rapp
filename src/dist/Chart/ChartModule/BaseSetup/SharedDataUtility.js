...
export let Shared_ChartPlotData = defaultchartData;
export let Shared_ChartDimension = defaultChartDimensionProp;
export let Shared_XScaleConfig = {};
export let Shared_YScaleConfig = {};
// import { createNestedProxy } from "./proxyfunction";
export let Shared_PlotInfo = {};
export let Shared_ButtonProp = {};
export let Shared_XYrelation = {};
// export let Shared_yaxistag: XscaleYscaleRelation = {};
export let Shared_ChartBaseData = {
    plotName: new Set(),
    xscaleTag: new Set(),
    yscaleTag: new Set(),
    yaxisTag: new Set(),
};
export let Shared_yaxisProp = {};
export function updateChartPlotData(data) {
    Shared_ChartPlotData = data;
}
export function updateChartBaseProp(partialData) {
    Object.assign(Shared_ChartDimension, partialData);
}
export function updateYaxis(key, partialData, reset) {
    if (reset == true) {
        Shared_yaxisProp = {};
    }
    else {
        if (Shared_yaxisProp.hasOwnProperty(key)) {
            ...
            Shared_yaxisProp[key] = Object.assign(Object.assign({}, Shared_yaxisProp[key]), partialData);
        }
        else {
            // If the key does not exist, create a new DataToplotObjType object with the provided data
            Shared_yaxisProp[key] = Object.assign({ range: [0, 0], plotname: [], yscaleTag: [] }, partialData);
        }
    }
}
export function getAxisKeyForRangeValue(value) {
...
export function getPlotNamesAndYScaleTagsByYAxisTag() {
    const result = {};
        ...
        if (Shared_PlotInfo.hasOwnProperty(plotKey)) {
            ...
            if (plot.plotStatus) {
                ...
                if (yscaleTag in Shared_YScaleConfig) {
                    ...
                    result[yaxisTag].plotName.push(plot.plotName);
                    // Add the yscaleTag to the array if it is not already present
                    if (!result[yaxisTag].yscaleTag.includes(yscaleTag)) {
                        result[yaxisTag].yscaleTag.push(yscaleTag);
                    }
                }
            }
        }
...
export function getXscale() {
    ...
    if (this.xsaleType = 'Linear') {
        const tempdatalenght = Shared_ChartPlotData[this.xscaleDataTag].length;
        tempdomain = d3.extent([0, tempdatalenght]);
    }
    else {
        tempdomain = d3.extent(Shared_ChartPlotData[this.xscaleDataTag]);
    }
    // console.log("domain1",tempdomain);
    const XSCALE = this.xsaleType == "Linear"
        ...
        : this.xsaleType === "TimeScale"
            ...
            : d3
                ...
                .domain(tempdomain.map((d) => d.toString()));
...
export function getYscale() {
    ...
    if (this.yscaleDataTag == "ohlc") {
        ...
        domain = [
            d3.min(Shared_ChartPlotData["low"].slice(visiblerange[0], visiblerange[1])),
            d3.max(Shared_ChartPlotData["high"].slice(visiblerange[0], visiblerange[1])),
        ];
    }
    else if (this.yscaleDataTag == "fundaMultibarY") {
        const tempyArray = Object.values(Shared_ChartPlotData[this.yscaleDataTag]);
        // console.log(tempyArray);
        const fundaMultibarYCombined = [].concat(...tempyArray);
        visiblerange = this.xscaleVisibleRange[1] == 0 ? [0, tempyArray[0].length] : this.xscaleVisibleRange;
        domain = [
            d3.min(fundaMultibarYCombined.slice(visiblerange[0], visiblerange[1])),
            d3.max(fundaMultibarYCombined.slice(visiblerange[0], visiblerange[1])),
        ];
    }
        ...
        visiblerange = this.xscaleVisibleRange[1] == 0 ? [0, Shared_ChartPlotData[this.yscaleDataTag].length] : this.xscaleVisibleRange;
        domain = d3.extent(Shared_ChartPlotData[this.yscaleDataTag].slice(visiblerange[0], visiblerange[1]));
    }
    ...
    const paddedDomain = [domainArray[0] - domainPadding, domainArray[1] + domainPadding];
...
export function updateShared_PlotInfo(key, partialData) {
    // Check if the key already exists in Shared_DataToplot
    if (Shared_PlotInfo.hasOwnProperty(key)) {
        // Merge the partial data with the existing DataToplotObjType object
        Shared_PlotInfo[key] = Object.assign(Object.assign({}, Shared_PlotInfo[key]), partialData);
    }
    else {
        // If the key does not exist, create a new DataToplotObjType object with the provided data
        Shared_PlotInfo[key] = Object.assign({ plotStatus: true, plotName: "PlotName", xdata: () => [], ydata: () => [], xscaleTag: "Bot1", yscaleTag: "TL", plotType: "line", plotcolor: "red", buttontag: "no-button", buttonSvgIcon: "defaultsvg", tooltip: false, clipdata: true }, partialData);
    }
}
export function updateShared_XScaleConfig(key, partialData) {
    // Check if the key already exists in Shared_DataToplot
    if (Shared_XScaleConfig.hasOwnProperty(key)) {
        // Merge the partial data with the existing DataToplotObjType object
        Shared_XScaleConfig[key] = Object.assign(Object.assign({}, Shared_XScaleConfig[key]), partialData);
    }
    else {
        // If the key does not exist, create a new DataToplotObjType object with the provided data
        Shared_XScaleConfig[key] = Object.assign({ xsaleType: "Linear", xscaleTag: "bot", scaleSide: "Bottom", ticlavelmappedwith: "xindex", ypoint: 100, xscaleRange: [0, 100], xscaleDataTag: "xindex", zoomstatus: true, xscale: getXscale }, partialData);
    }
}
export function updateShared_YScaleConfig(key, partialData) {
    // Check if the key already exists in Shared_DataToplot
    if (Shared_YScaleConfig.hasOwnProperty(key)) {
        // Merge the partial data with the existing DataToplotObjType object
        Shared_YScaleConfig[key] = Object.assign(Object.assign({}, Shared_YScaleConfig[key]), partialData);
    }
    else {
        // If the key does not exist, create a new DataToplotObjType object with the provided data
        Shared_YScaleConfig[key] = Object.assign({ yscaleTag: "bot", yaxisTag: "1main", scaleSide: "Left", xpoint: 100, 
            // yscaleRange: [0, 100],
            yaxisrange: null, yscaleDataTag: "close", xscaleVisibleRange: [0, 10], zoomstatus: true, autozoom: true, ydomaindata: [0, 0], yscale: getYscale, yzoomtransform: d3.zoomIdentity }, partialData);
    }
}
...
export function getUniquePlotsWithStatusTrue(plotInfo) {
    const uniquePlotNames = new Set();
    const uniquexscaletags = new Set();
    const uniqueyscaletags = new Set();
    const uniqueyaxistags = new Set();
    Object.values(plotInfo)
        ...
        uniqueyaxistags.add(Shared_YScaleConfig[plot.yscaleTag].yaxisTag);
    });
    return { plotName: uniquePlotNames, xscaleTag: uniquexscaletags, yscaleTag: uniqueyscaletags, yaxisTag: uniqueyaxistags };
}
export function updateSharedChartData(data) {
    for (const key in data) {
        const typedKey = key; // Cast key to keyof ChartBaseData
...
export function getPlotStatusByButtonTag() {
...
export function collectKeysByButtonId(buttonId) {
    const keys = [];
...
export function updateYScaleConfigByKey(keyName, value, partialData) {
    ...
    yScaleConfigEntries.forEach(([key, config]) => {
        Shared_YScaleConfig[key] = Object.assign(Object.assign({}, config), partialData);
    });
    // console.log("Shared_YScaleConfig",Shared_YScaleConfig);
}
export function getYaxisRatio(yaxistags) {
    // resetYaxisrange()
...
export function generateRelationObject() {
    ...
    Object.values(Shared_PlotInfo).forEach((item) => {
        ...
        if (item.plotStatus) {
            ...
            if (!Shared_XYrelation[xscaleTag].includes(item.yscaleTag)) {
                Shared_XYrelation[xscaleTag].push(item.yscaleTag);
            }
        }
    });
}
export function groupDataByPlotType() {
    const groupedData = {};
